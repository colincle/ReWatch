...
char const * tinyfd_getGlobalChar(char const * aCharVariableName)  /* to be called from C# (you don't need this in C or C++) */
{
	if(!aCharVariableName || !strlen(aCharVariableName))
	{
		...
		if(!strcmp(aCharVariableName, "tinyfd_version"))
		{
			return tinyfd_version;
		}
		else
			if(!strcmp(aCharVariableName, "tinyfd_needs"))
			{
				return tinyfd_needs;
			}
			else
				if(!strcmp(aCharVariableName, "tinyfd_response"))
				{
...
int tinyfd_getGlobalInt(char const * aIntVariableName)  /* to be called from C# (you don't need this in C or C++) */
{
	if(!aIntVariableName || !strlen(aIntVariableName))
	{
		...
		if(!strcmp(aIntVariableName, "tinyfd_verbose"))
		{
			return tinyfd_verbose ;
		}
		else
			if(!strcmp(aIntVariableName, "tinyfd_silent"))
			{
				return tinyfd_silent ;
			}
			else
				if(!strcmp(aIntVariableName, "tinyfd_allowCursesDialogs"))
				{
					return tinyfd_allowCursesDialogs ;
				}
				else
					if(!strcmp(aIntVariableName, "tinyfd_forceConsole"))
					{
	...
	/* else if (!strcmp(aIntVariableName, "tinyfd_assumeGraphicDisplay")) return tinyfd_assumeGraphicDisplay ; */
						...
						if(!strcmp(aIntVariableName, "tinyfd_winUtf8"))
						{
...
static int dirExists(char const * aDirPath)
{
	...
	if(tinyfd_winUtf8)
	{
		...
		if(_stat(aDirPath, &lInfo) != 0)
#else
	else
		if(_stat64(aDirPath, &lInfo) != 0)
#endif
			return 0;
		else
			if(lInfo.st_mode & S_IFDIR)
    ...
    int aDefaultButton) /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
	...
	if(aIconType && !wcscmp(L"warning", aIconType))
	{
		...
		if(aIconType && !wcscmp(L"error", aIconType))
		{
			aCode = MB_ICONERROR;
		}
		else
			if(aIconType && !wcscmp(L"question", aIconType))
			{
	...
	if(aDialogType && !wcscmp(L"okcancel", aDialogType))
	{
		...
		if(aDialogType && !wcscmp(L"yesno", aDialogType))
		{
			aCode += MB_YESNO;
	...
	if((lBoxReturnValue == IDNO) && (aDialogType && !wcscmp(L"yesnocancel", aDialogType)))
	{
		...
		if((lBoxReturnValue == IDOK) || (lBoxReturnValue == IDYES))
		{
			return 1;
		}
		else
...
static char * dialogNameOnly(void)
{
	...
	if(lDialogName[0] == '*')
	{
		if(!tinyfd_allowCursesDialogs)
		{
			...
			if(tfd_isDarwin() && * strcpy(lDialogName, "/opt/local/bin/dialog")
			        && detectPresence(lDialogName))
			{}
			else
				if(* strcpy(lDialogName, "dialog")
				        && detectPresence(lDialogName))
				{}
				else
				{
...
static char * terminalName(void)
{
	...
	if(lTerminalName[0] == '*')
	{
		if(detectPresence("bash"))
		{
			...
			if(strlen(dialogNameOnly()) || whiptailPresentOnly())
			{
		...
		if(tfd_isDarwin())
		{
			...
			if(* strcpy(lTerminalName, "xterm")   /*good (small without parameters)*/
			        && detectPresence(lTerminalName))
			{
				strcat(lTerminalName, " -fa 'DejaVu Sans Mono' -fs 10 -title tinyfiledialogs -e ") ;
				strcat(lTerminalName, lShellName) ;
				...
				if(* strcpy(lTerminalName, "terminator")   /*good*/
				        && detectPresence(lTerminalName))
				{
					...
					if(* strcpy(lTerminalName, "lxterminal")   /*good*/
					        && detectPresence(lTerminalName))
					{
						...
						if(* strcpy(lTerminalName, "konsole")   /*good*/
						        && detectPresence(lTerminalName))
						{
							strcat(lTerminalName, " -e ") ;
							strcat(lTerminalName, lShellName) ;
							...
							if(* strcpy(lTerminalName, "kterm")   /*good*/
							        && detectPresence(lTerminalName))
							{
								strcat(lTerminalName, " -e ") ;
								strcat(lTerminalName, lShellName) ;
								...
								if(* strcpy(lTerminalName, "tilix")   /*good*/
								        && detectPresence(lTerminalName))
								{
									strcat(lTerminalName, " -e ") ;
									strcat(lTerminalName, lShellName) ;
									...
									if(* strcpy(lTerminalName, "xfce4-terminal")   /*good*/
									        && detectPresence(lTerminalName))
									{
										strcat(lTerminalName, " -x ") ;
										strcat(lTerminalName, lShellName) ;
										...
										if(* strcpy(lTerminalName, "mate-terminal")   /*good*/
										        && detectPresence(lTerminalName))
										{
											strcat(lTerminalName, " -x ") ;
											strcat(lTerminalName, lShellName) ;
											...
											if(* strcpy(lTerminalName, "Eterm")   /*good*/
											        && detectPresence(lTerminalName))
											{
												strcat(lTerminalName, " -e ") ;
												strcat(lTerminalName, lShellName) ;
												...
												if(* strcpy(lTerminalName, "evilvte")   /*good*/
												        && detectPresence(lTerminalName))
												{
													strcat(lTerminalName, " -e ") ;
													strcat(lTerminalName, lShellName) ;
													...
													if(* strcpy(lTerminalName, "pterm")   /*good (only letters)*/
													        && detectPresence(lTerminalName))
													{
														strcat(lTerminalName, " -e ") ;
														strcat(lTerminalName, lShellName) ;
														...
														if(* strcpy(lTerminalName, "gnome-terminal")
		...
		if(tfd_zenityPresent())
		{
			...
			if(fgets(lBuff, sizeof(lBuff), lIn) != NULL)
			{
				if(atoi(lBuff) >= 3)
				{
					...
					if((atoi(lBuff) == 2) && (atoi(strtok(lBuff, ".") + 2) >= 32))
					{
						lZenity3Present = 2 ;
					}
		...
		if(osascriptPresent())
		{
			...
			if(speakertestPresent())
			{
				...
				if(ffplayPresent())
				{
					...
					if(playPresent())  /* play is part of sox */
					{
		...
		if(tfd_kdialogPresent())
		{
			...
			if(tfd_zenityPresent() || tfd_matedialogPresent() || tfd_shellementaryPresent() || tfd_qarmaPresent())
			{
				...
				if(tfd_yadPresent())
				{
					...
					if(!gxmessagePresent() && !gmessagePresent() && !gdialogPresent() && !xdialogPresent() && tkinter3Present())
					{
						if(aTitle && !strcmp(aTitle, "tinyfd_query"))
						{
...
if res is False :\n\tprint(0)\n\
						...
						if(!gxmessagePresent() && !gmessagePresent() && !gdialogPresent() && !xdialogPresent() && tkinter2Present())
						{
							if(aTitle && !strcmp(aTitle, "tinyfd_query"))
							{
...
if res is False :\n\tprint 0\n\
							...
							if(gxmessagePresent() || gmessagePresent() || (!gdialogPresent() && !xdialogPresent() && xmessagePresent()))
							{
								if(gxmessagePresent())
								{
									if(aTitle && !strcmp(aTitle, "tinyfd_query"))
								...
								if(xdialogPresent() || gdialogPresent() || dialogName() || whiptailPresent())
								{
									if(gdialogPresent())
									{
										if(aTitle && !strcmp(aTitle, "tinyfd_query"))
...
tinyfdRes=$(cat /tmp/tinyfd.txt);echo $tinyfdBool$tinyfdRes") ;
				...
				tinyfdRes=$(cat /tmp/tinyfd.txt);echo $tinyfdBool$tinyfdRes") ;
									...
									if(!isTerminalRunning() && terminalName())
									{
										if(aTitle && !strcmp(aTitle, "tinyfd_query"))
										{
											strcpy(tinyfd_response, "basicinput");
										...
										if(!isTerminalRunning() && pythonDbusPresent() && !strcmp("ok", aDialogType))
										{
											if(aTitle && !strcmp(aTitle, "tinyfd_query"))
											{
											...
											if(!isTerminalRunning() && (perlPresent() >= 2) && !strcmp("ok", aDialogType))
											{
												if(aTitle && !strcmp(aTitle, "tinyfd_query"))
												{
...
'org.freedesktop.Notifications');");
												...
												if(!isTerminalRunning() && notifysendPresent() && !strcmp("ok", aDialogType))
												{
			...
			if(tfd_zenityPresent() || tfd_matedialogPresent() || tfd_shellementaryPresent() || tfd_qarmaPresent())
			{
				...
				if(tfd_yadPresent())
				{
					...
					if(gxmessagePresent() || gmessagePresent())
					{
						...
						if(!gdialogPresent() && !xdialogPresent() && tkinter3Present())
						{
							if(aTitle && !strcmp(aTitle, "tinyfd_query"))
							{
								strcpy(tinyfd_response, "python3-tkinter");
							...
							if(!gdialogPresent() && !xdialogPresent() && tkinter2Present())
							{
								if(aTitle && !strcmp(aTitle, "tinyfd_query"))
								{
									strcpy(tinyfd_response, "python2-tkinter");
...
frontmost of process \\\"Python\\\" to true' ''');");
								...
								if(gdialogPresent() || xdialogPresent() || dialogName() || whiptailPresent())
								{
									if(gdialogPresent())
									{
										if(aTitle && !strcmp(aTitle, "tinyfd_query"))
		...
		tinyfdRes=$(cat /tmp/tinyfd.txt);echo $tinyfdBool$tinyfdRes") ;
									...
									if(! isTerminalRunning() && terminalName())
									{
										...
										if(!gWarningDisplayed && ! isTerminalRunning() && ! terminalName())
										{
											gWarningDisplayed = 1 ;
											tinyfd_messageBox(gTitle, tinyfd_needs, "ok", "warning", 0);
		...
		if(tfd_kdialogPresent())
		{
			...
			if(tfd_zenityPresent() || tfd_matedialogPresent() || tfd_shellementaryPresent() || tfd_qarmaPresent())
			{
				...
				if(tfd_yadPresent())
				{
					...
					if(!xdialogPresent() && tkinter3Present())
					{
						if(aTitle && !strcmp(aTitle, "tinyfd_query"))
						{
							strcpy(tinyfd_response, "python3-tkinter");
						...
						if(!xdialogPresent() && tkinter2Present())
						{
							if(aTitle && !strcmp(aTitle, "tinyfd_query"))
							{
								strcpy(tinyfd_response, "python2-tkinter");
